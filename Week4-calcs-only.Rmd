...
hh_data <- read_csv("households.csv") %>%
  mutate(struct_type = case_when(
    ...
    BLD_label=="One-family house detached" ~ "SFhome",
...
In the results above, the coefficient estimate for `age_struct` is -1.4654. This tells us that for each additional year in a building's age, the rent for the homes in that building decrease by $1.47, on average, controlling for differences in the number of bedrooms, differences in structure type, and whether the residents are proficient in English.

The coefficient estimate of `BDSP` is 108.9489. This tells us that each additional bedroom in a home is associated with an average increase in rent of $109, on average, controlling for the other variables in the model.
...
For structure type, everything is compared to a single-family home (you can tell because that's the category that isn't shown in the coefficients table). The coefficient for buildings with more than 49 units is 45.5194. This means that, on average and controlling for everything else in the model, the rent for a home in a building with more than 49 units will be $45.52 more than the rent would be for a single-family home. 

The coefficient for a mobile home or vehicle is -769.7138, which indicated that the rent for a mobile home would be $769.71 less than for a comparable single-family home.

The coefficient for English language proficiency is 117.5494, meaning that households in which at least one person over the age of 14 speaks English either exclusively or very well pay an average of $117.55 more in rent than households with limited English proficiency do, controlling for everything else in the model.
