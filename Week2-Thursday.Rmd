...
x_count <- seq(0, 100, by = 1)
y_poisson <- dpois(x_count, lambda = 20)
y_nbinom <- dnbinom(x_count, size = 100, prob = 0.75)
y_log <- dlnorm(x_count, meanlog = 3, sdlog = 0.5)
...
data_log <- tibble(x = x_count, y = y_log)
...
But in this course, we'll be focusing data that's normally distributed.
...
Spread is commonly measured in terms of *standard deviation*, which you can think of as the average difference bewteen all all the values in your dataset and the average value (that's not quite what the math is, but that's basically it). It's a measure of how much variation there is in the data - if all your observations had the same value, the standard deviation would be zero.
...
Here are the standard deviations for a normal distribution.
...
And here is the interquartile range:

```{r, echo = FALSE}
ggplot(data = data_wider, aes(x = x, y = y)) +
  geom_line() +
  geom_vline(xintercept = -2*0.67448) +
  geom_vline(xintercept = 2*0.67448) +
